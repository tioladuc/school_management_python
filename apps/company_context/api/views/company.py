    ...
    def get(self, request):
        return Response(
            CompanyResponseSerializer(
                services()["company"].search(SearchCompanyQuery("", None)),# [services()["company"].to_dto(c) for c in services()["company"].search(SearchCompanyQuery("", None))],
                many=True
            ).data            
        )
...
class CompanyDetailView(APIView):
    def get(self, request, company_id):
        return Response(
            CompanyResponseSerializer(
                services()["company"].get(GetCompanyQuery(company_id))
            ).data
        )

    def put(self, request, company_id):
        s = UpdateCompanyRequestSerializer(data=request.data)
        s.is_valid(raise_exception=True)
        r = services()["company"].update(
            UpdateCompanyCommand(company_id, **s.validated_data)
        )
        return Response(CompanyResponseSerializer(r).data)
...
class CompanyActivateView(APIView):
    def post(self, request, company_id):
        return Response(
            CompanyResponseSerializer(
                services()["company"].activate(ActivateCompanyCommand(company_id))
            ).data
        )
...
class CompanySuspendView(APIView):
    def post(self, request, company_id):
        return Response(
            CompanyResponseSerializer(
                services()["company"].suspend(SuspendCompanyCommand(company_id))
            ).data
        )
