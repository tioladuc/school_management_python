...
class SchoolDetailView(APIView):
    def get(self, request, school_id):
        return Response(
            SchoolResponseSerializer(
                services()["school"].get(GetSchoolQuery(school_id))
            ).data
        )

    def put(self, request, school_id):
        s = UpdateSchoolRequestSerializer(data=request.data)
        s.is_valid(raise_exception=True)
        r = services()["school"].update(
            UpdateSchoolCommand(school_id, **s.validated_data)
        )
        return Response(SchoolResponseSerializer(r).data)
...
class CompanySchoolListView(APIView):
    def get(self, request, company_id):
        return Response(
            SchoolResponseSerializer(
                services()["school"].list(ListCompanySchoolsQuery(company_id)),
                many=True,
            ).data
        )
